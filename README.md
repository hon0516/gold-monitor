...
基于 `FastAPI + APScheduler + SQLite + SMTP` 的积存金价格监测服务。项目会定时拉取积存金最新价格，计算最近 1 小时窗口内的高低差和振幅，并在达到阈值时发送邮件提醒。

服务内置一个轻量 Web 配置页面，启动后可以在浏览器中配置 SMTP、收件人、告警阈值、运行状态和告警记录，适合部署为局域网或服务器上的常驻监控服务。

## 主要功能

- 定时监测积存金价格，默认 30 秒检测一次。
...
- 提供 REST API 和 WebSocket 更新接口，方便二次开发或接入其它系统。
